...
// ── Scenario 1: Full project ──
{
  ...
  run("S1: Empty project — returns empty-ish string with no sections", () => {
...
// ── Scenario 7: Session state preview (under 20 lines) ──
{
  ...
  const output = handleSessionCreated(root)
  run("S7: Source does not include session state preview", () => {
    assert.ok(!output.includes("ACTIVE SESSION STATE"), "no session state in handleSessionCreated output")
  })
  cleanup(root)
}
...
// ── Scenario 8: Session state preview (over 20 lines) ──
{
  ...
  const output = handleSessionCreated(root)
  run("S8: Source does not include session state for long files", () => {
    assert.ok(!output.includes("ACTIVE SESSION STATE"), "no session state in handleSessionCreated output")
  })
  cleanup(root)
}
