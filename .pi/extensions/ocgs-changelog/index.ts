...
function getUnreleasedCommits(): {
	...
	try {
		const log = execSync(
			"git log --oneline --no-decorate HEAD --not --tags 2>/dev/null || git log --oneline -20",
			{
				encoding: "utf-8",
			},
		);
...
function generateChangelogPreview(
		...
		handler: async (_args, ctx) => {
			...
			if (ctx.mode === "tui") {
				...
				if (result?.action === "accept") {
					...
					ctx.ui.notify(
						"Edit the changelog in your editor and run /changelog again.",
						"info",
					);
				}
			} else {
