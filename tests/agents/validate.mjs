...
function validateAgents() {
	...
	for (const file of files) {
		...
		for (const section of REQUIRED_AGENT_SECTIONS) {
			if (!content.includes(section)) {
				...
				if (
					...
					(content.includes("Reports to:") || content.includes("Reports to"))
				)
					continue;
				if (
					section === "What This Agent Must NOT Do" &&
		...
		const missingOptional = OPTIONAL_AGENT_SECTIONS.filter(
		...
		const effectiveStatus =
			status === "FAIL" && isException ? "EXCEPTED" : status;

		if (effectiveStatus === "EXCEPTED") {
			passed++;
			warnings.push(...issues.map((i) => `[EXCEPTED] ${i}`));
			warnings.push(
				"This agent is in the exceptions list — remove from AGENT_EXCEPTIONS when these gaps are closed.",
			);
		} else if (status === "PASS" && isException) {
			passed++;
			warnings.push(
				"Agent passes all checks but is still in AGENT_EXCEPTIONS — remove from exceptions list.",
			);
		} else if (status === "PASS") {
			passed++;
		} else {
		...
		if (fm.error) {
			...
			if (fm.data.agent) {
				...
				if (!agentNames.includes(refAgent)) {
					issues.push(
						`Agent reference '${refAgent}' does not match any agent file (valid: ${agentNames.length} agents)`,
					);
				}
			}
			...
			const subagentRefs =
				content.match(/subagent_type:\s*`?([a-z][\w-]+)`?/g) || [];
			for (const ref of subagentRefs) {
				...
				if (!agentNames.includes(agent)) {
					issues.push(
						`Content references unknown agent '${agent}' via subagent_type`,
					);
				}
			}
		}
	...
	for (const file of files) {
		...
		if (fm.error) {
			...
			if (fm.data.skill && !skillNames.includes(fm.data.skill)) {
				issues.push(
					`Skill reference '${fm.data.skill}' does not match any skill directory`,
				);
			}
			...
			if (fm.data.category && !validCategories.includes(fm.data.category)) {
				issues.push(
					`Invalid category '${fm.data.category}'. Valid: ${validCategories.join(", ")}`,
				);
			}
		}
		...
		for (const dir of skillDirs) {
			...
			for (const entry of entries) {
				if (entry.toLowerCase().startsWith("readme")) {
					issues.push(
						`Pattern violation: skills/${dir}/${entry} — skill dirs should not contain README files`,
					);
				}
			}
		}
...
function generateReport(sections) {
	...
	for (const section of sections) {
		...
		if (exempted.length > 0) {
			report += "### Excepted\n\n";
			report +=
				"These agents are in the known exceptions list (AGENT_EXCEPTIONS). They are validated but do not block CI. Remove from the exceptions list when fixed.\n\n";
			for (const e of exempted) {
				const exceptedIssues = (e.warnings || []).filter((w) =>
					w.startsWith("[EXCEPTED]"),
				);
		...
		// Show warnings
		const itemsWithWarnings = results.filter(
