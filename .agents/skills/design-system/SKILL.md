...
- Read `docs/framework/technical-preferences.md` to identify the engine and version
...
Use the template structure from `docs/framework/templates/game-design-document.md`:
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   `question`. **NEVER use plain text. NEVER skip this step.**
...
- Surface any disagreements between agents to the user via `question`
...
Before finalizing the GDD, spawn `creative-director` via Task using gate **CD-GDD-ALIGN** (`docs/framework/director-gates.md`).
